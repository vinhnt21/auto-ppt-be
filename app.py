...
def create_powerpoint():
    try:
        ...
        ensure_directory_exists("/var/data/slides")
        ...
        try:
            ...
            for i in range(len(slide_content)):
                ...
                if i != 0 and i != len(slide_content) - 1:
                    img_url = (
                        ...
                        if slide_with_img
                        else "/var/data/slides/_placeholder.png"
                    )
                    slide_content[i] += f"\n\n![img]({img_url})"
            ...
            markdown_to_pptx(md_content, f"/var/data/slides/{slide_name}.pptx")
...
def download_file(filename):
    try:
        return send_from_directory("/var/data/slides", filename, as_attachment=True)
    except Exception as e:
